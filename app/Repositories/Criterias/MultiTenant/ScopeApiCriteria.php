...
class ScopeApiCriteria implements CriteriaInterface
{
    ...
    {
        $companies = Auth::guard('api')->user()->groupCompany->companies()->pluck('id')->toArray();
    ...
    {
        return get_class($model->getModel()) == 'Delos\Dgp\Entities\Activity' && (str_contains(app('request')->url(), '/rests'));
    }
    ...
    private function ApplyScopeInGroupCompany($model)
    {
        $model = $model->whereHas('groupCompany', function (Builder $query) {
            $query->whereIn('id', Auth::guard('api')->user()->groupCompany->id);
        });
